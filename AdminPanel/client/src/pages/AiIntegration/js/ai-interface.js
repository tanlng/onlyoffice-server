...
(function (window, undefined) {
  ...
  function onInit(source) {
    ...
    const providers = Object.keys(settings.providers).map(function (key) {
      return settings.providers[key];
    });
    var model = {id: '', name: '', provider: '', capabilities: 0};
    if (tmpModel) {
      model = settings.models.find(function (model) {
        return model.name === tmpModel.name;
      });
      tmpModel = null;
    }
    var data = {
  ...
  function onChangeModel(data) {
    ...
    if (!isFoundModel) {
      if (data.capabilities === undefined) data.capabilities = 0xffff;
